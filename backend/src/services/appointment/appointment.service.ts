...
export class AppointmentService {
  static async createAppointment(
    createAppointmentDTO: CreateAppointmentDTO): Promise<any> {
    const { doctorId, userId, date: dateDTO, time, notes, reason } = createAppointmentDTO;
  ...
  private static generateAvailableSlots(date: Date, workingHours: any, bookedAppointments: any[]): Date[] {
    ...
    while (currentSlot <= endTime) {
      if (
        ...
        if (!isBooked) {
          const dateFormattedBogota = moment.tz(currentSlot, 'America/Bogota').format('YYYY-MM-DD HH:mm');
