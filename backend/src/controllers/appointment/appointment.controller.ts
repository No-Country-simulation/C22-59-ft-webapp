...
class AppointmentController {
	async createAppointment(req: Request, res: Response) {
      ...
      if(error instanceof ErrorAppointment){
	...
	async getAvailableSlots(req: Request, res: Response) {
		try {
			...
			httpResponse.Error(
				res,
				error.message || "Error fetching available slots"
			);
		}
	}
	...
	async updateAppointmentStatus(req: Request, res: Response) {
		try {
			...
			httpResponse.Error(
				res,
				error.message || "Error updating appointment status"
			);
		}
	}
	...
	async getUserAppointments(req: Request, res: Response) {
		try {
			...
			httpResponse.Error(
				res,
				error.message || "Error fetching user appointments"
			);
		}
	}
	...
	async getDoctorAppointments(req: Request, res: Response) {
		try {
			...
			httpResponse.Error(
				res,
				error.message || "Error fetching doctor appointments"
			);
		}
	}
